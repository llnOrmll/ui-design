...
The `loadShader()` function takes as input the WebGL context, the shader type, and the source code, then creates and compiles the shader as follows:
  ...
  1. A new shader is created by calling [`gl.createShader()`](/en-US/docs/Web/API/WebGLRenderingContext/createShader "gl.createShader\(\)").
  2. The shader's source code is sent to the shader by calling [`gl.shaderSource()`](/en-US/docs/Web/API/WebGLRenderingContext/shaderSource "gl.shaderSource\(\)").
  3. Once the shader has the source code, it's compiled using [`gl.compileShader()`](/en-US/docs/Web/API/WebGLRenderingContext/compileShader "gl.compileShader\(\)").
  4. To check to be sure the shader successfully compiled, the shader parameter `gl.COMPILE_STATUS` is checked. To get its value, we call [`gl.getShaderParameter()`](/en-US/docs/Web/API/WebGLRenderingContext/getShaderParameter "gl.getShaderParameter\(\)"), specifying the shader and the name of the parameter we want to check (`gl.COMPILE_STATUS`). If that's `false`, we know the shader failed to compile, so show an alert with log information obtained from the compiler using [`gl.getShaderInfoLog()`](/en-US/docs/Web/API/WebGLRenderingContext/getShaderInfoLog "gl.getShaderInfoLog\(\)"), then delete the shader and return `null` to indicate a failure to load the shader.
...
We'll do that using a function we call `initBuffers()`, which we will implement in a separate [JavaScript module](/en-US/docs/Web/JavaScript/Guide/Modules). As we explore more advanced WebGL concepts, this module will be augmented to create more â and more complex â 3D objects.
...
This routine is pretty simplistic given the basic nature of the scene in this example. It starts by calling the `gl` object's [`createBuffer()`](/en-US/docs/Web/API/WebGLRenderingContext/createBuffer "createBuffer\(\)") method to obtain a buffer into which we'll store the vertex positions. This is then bound to the context by calling the [`bindBuffer()`](/en-US/docs/Web/API/WebGLRenderingContext/bindBuffer "bindBuffer\(\)") method.

Once that's done, we create a JavaScript array containing the position for each vertex of the square plane. This is then converted into an array of floats and passed into the `gl` object's [`bufferData()`](/en-US/docs/Web/API/WebGLRenderingContext/bufferData "bufferData\(\)") method to establish the vertex positions for the object.
...
### See also
  ...
  * [ Previous ](/en-US/docs/Web/API/WebGL_API/Tutorial/Getting_started_with_WebGL)
  * [ Next ](/en-US/docs/Web/API/WebGL_API/Tutorial/Using_shaders_to_apply_color_in_WebGL)
...
This page was last modified on â¨Dec 8, 2024â© by [MDN contributors](/en-US/docs/Web/API/WebGL_API/Tutorial/Adding_2D_content_to_a_WebGL_context/contributors.txt). 

[View this page on GitHub](https://github.com/mdn/content/blob/main/files/en-us/web/api/webgl_api/tutorial/adding_2d_content_to_a_webgl_context/index.md?plain=1 "Folder: â¨en-us/web/api/webgl_api/tutorial/adding_2d_content_to_a_webgl_contextâ© \(Opens in a new tab\)") â¢ [Report a problem with this content](https://github.com/mdn/content/issues/new?template=page-report.yml&mdn-url=https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FWebGL_API%2FTutorial%2FAdding_2D_content_to_a_WebGL_context&metadata=%3C%21--+Do+not+make+changes+below+this+line+--%3E%0A%3Cdetails%3E%0A%3Csummary%3EPage+report+details%3C%2Fsummary%3E%0A%0A*+Folder%3A+%60en-us%2Fweb%2Fapi%2Fwebgl_api%2Ftutorial%2Fadding_2d_content_to_a_webgl_context%60%0A*+MDN+URL%3A+https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FWebGL_API%2FTutorial%2FAdding_2D_content_to_a_WebGL_context%0A*+GitHub+URL%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fblob%2Fmain%2Ffiles%2Fen-us%2Fweb%2Fapi%2Fwebgl_api%2Ftutorial%2Fadding_2d_content_to_a_webgl_context%2Findex.md%0A*+Last+commit%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fcommit%2Fec7b3262bcd4a03a09bab0c57e3de4b51d4de4f8%0A*+Document+last+modified%3A+2024-12-08T06%3A12%3A36.000Z%0A%0A%3C%2Fdetails%3E "This will take you to GitHub to file a new issue.")
