...
- Creates and returns a new canvas pattern object. `image` is the source of the image (that is, an `HTMLImageElement`, another `SVGImageElement` or a `HTMLCanvasElement`, an `OffscreenCanvas` or a `HTMLVideoElement`, or an `ImageBitmap`, `VideoFrame`). `type` is a string indicating how to use the image.
...
*Documentation extracted from [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Applying_styles_and_colors)*