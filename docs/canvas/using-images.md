...
If you're using one external image, this can be a good approach, but once you want to use many images or lazy-load resources, you probably need to wait for all the files to be available before drawing to the canvas.

The examples below that deal with multiple images use an async function and `Promise.all` to wait for all images to load before calling `drawImage()`:
...
Each `<img>` is assigned an ID attribute, so we have one for a `source` and one for the `frame`, which makes them easy to select using `document.getElementById()`. We're using `Promise.all` to wait for all images to load before calling `drawImage()`.
...
*Documentation extracted from [MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Using_images)*